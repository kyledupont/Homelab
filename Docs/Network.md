...
- **Resolved 2026-07-28: Proxmox re-IP'd from `.209` (inside the router's DHCP range) to `192.168.1.10` (the safe `.2`-`.63` static range), matching every other host in the lab.** Changed via the Proxmox web UI's Network config (`vmbr0`), applied directly rather than taking the DHCP-reservation shortcut — blast radius turned out to be contained to Proxmox's own management IP; every VM kept its own independent static IP on the same bridge and needed no changes. Everything referencing the old IP (Terraform's `proxmox_endpoint`, the homepage dashboard link, docs) was updated in the same pass.
